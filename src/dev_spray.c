...
/* dev_spray: device spray-coverage test (KDP geometry!).
 * Same trigger + return->spray->park->fire. argv[1] = lane:
...
static void *waiter_fn(void *arg)
{
    ...
    spray_done = 1; /* S22U shape: return->spray->park->fire */
    while (!fire_done)
        sched_yield();
...
int main(int argc, char **argv)
{
    ...
    if (argc > 1)
        lane = atoi(argv[1]);
    printf("dev_spray: o1s KDP-geometry lane=%d\n", lane);
