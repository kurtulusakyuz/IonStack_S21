...
int verify_p0_pipe_oracle_gate(void) {
  ...
  /* Olcum: pipe doluluk durumu (state degismez, sadece FIONREAD). */
  for (size_t dbg_pipe = 0; dbg_pipe < 4 && dbg_pipe < PIPE_RECLAIM;
       dbg_pipe++) {
    int avail = -1;
    int rc = ioctl(pipe_fds_reclaim[dbg_pipe][0], FIONREAD, &avail);
    pr_info("slide gate dbg pipe=%zu fionread_rc=%d avail=%d\n",
            dbg_pipe, rc, avail);
  }
#endif
#if defined(APP_REQUIRE_FRESH_P0_SESSION) && APP_REQUIRE_FRESH_P0_SESSION
