...
__attribute__((constructor)) static void load(void) {
  ...
  started = 1;
  set_unbuffer();
  /* Fork-safe logging: shell redirects ("> file", truncate mode) share one
   * file offset across fork(), so parent/child logs overwrite each other
   * (NUL holes, lost lines). O_APPEND makes every write atomic-append. */
  {
    int fl1 = fcntl(STDOUT_FILENO, F_GETFL, 0);
    if (fl1 >= 0)
      fcntl(STDOUT_FILENO, F_SETFL, fl1 | O_APPEND);
    int fl2 = fcntl(STDERR_FILENO, F_GETFL, 0);
    if (fl2 >= 0)
      fcntl(STDERR_FILENO, F_SETFL, fl2 | O_APPEND);
  }
