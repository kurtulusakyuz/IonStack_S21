...
ssize_t configfs_read_once(int fd, uintptr_t target, void *data, size_t len) {
  ...
  if (!page) {
    ...
    pr_info("slide diag read stage=nopage target=%016zx len=%zu\n",
            target, len);
#endif
    return -1;
  }
  ...
  if (set_ret != 0) {
#ifdef SLIDE_SIGRETURN_SPRAY
    pr_info("slide diag read stage=setfail target=%016zx len=%zu "
            "set_errno=%d page=%016zx pos=%lx\n",
            target, len, set_errno, page, (long)pos);
#endif
    errno = set_errno;
    return -1;
  }
...
#ifdef SLIDE_SIGRETURN_SPRAY
  if (rd != (ssize_t)len) {
    pr_info("slide diag read stage=short target=%016zx len=%zu rd=%zd "
            "err=%d page=%016zx pos=%lx\n",
            target, len, rd, errno, page, (long)pos);
  }
#endif
  return rd;
}
